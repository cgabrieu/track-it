    ...
    return (
        <>
            ...
            <Container color="#E5E5E5">
                <AddMyHabitsContainer>
                    <h1>Meus hábitos</h1>
                    <button onClick={() => setShowHabitForm(true)} >+</button>
                </AddMyHabitsContainer>
                {showHabitForm && <FormAddHabit user={user} setShowHabitForm={setShowHabitForm} setSuccess={setSuccess}/>}

                {listHabits.length > 0 
                ? listHabits.map(e => <ItemHabit key={e.id} id={e.id} name={e.name} days={e.days} setSuccess={setSuccess}/>) 
...
const AddMyHabitsContainer = styled.div`
    width: 100%;
    display: flex;
